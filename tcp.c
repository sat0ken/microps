    ...
    switch (pcb->state) {
        ...
        case TCP_STATE_LAST_ACK:
            ...
            if (!seg->len) {
                ...
                if (!pcb->rcv.wnd) {
                    errorf("tcp packet cant accept");
                } else {
                    if ((pcb->rcv.nxt <= seg->seq && seg->seq < pcb->rcv.nxt + pcb->rcv.wnd) ||
                        (pcb->rcv.nxt <= seg->seq + seg->len - 1 && seg->seq + seg->len - 1 < pcb->rcv.nxt + pcb->rcv.nxt)) {
                        acceptable = 1;
                    }
                }
            }
...
tcp_close(int id)
{
    ...
    switch (pcb->state) {
        ...
        case TCP_STATE_TIME_WAIT:
            ...
            break;
        default:
            errorf("unknown state: %d", pcb->state);
            mutex_unlock(&mutex);
            return -1;
    }
...
RETRY:
    switch (pcb->state) {
        ...
        case TCP_STATE_LAST_ACK:
            ...
            return -1;
        default:
            errorf("unknown statu: %u", pcb->state);
            mutex_unlock(&mutex);
            return -1;
    }
    mutex_unlock(&mutex);
