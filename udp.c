...
udp_input(const uint8_t *data, size_t len, ip_addr_t src_addr, ip_addr_t dst_addr, struct ip_iface *iface)
{
    ...
    udp_dump(data, len);
    mutex_lock(&mutex);
    pcb = udp_pcb_select(dst_addr, hdr->dst_port);
    if (!pcb) {
        mutex_unlock(&mutex);
        return;
    }
    ...
    entry->foreign.addr = src_addr;
    entry->foreign.port = hdr->src_port;
...
udp_sendto(int id, uint8_t *data, size_t len, struct ip_endpoint *foreign)
{
    ...
    if (local.addr == IP_ADDR_ANY) {
        ...
        debugf("select local address, src_addr=%s", ip_addr_ntop(local.addr, addr, sizeof(addr)));
    }
    if (!pcb->local.port) {
